# Willkommen in core

## Schnellstart

- [[01 Inbox/Eingang]] — Neue Ideen schnell erfassen
- [[00 Kontext/Über mich]] — Persönliches Profil

## Aktive Projekte

- [[02 Projekte/KiaChargeNav]]
- [[02 Projekte/IHK-Bewertungssoftware]]
- [[02 Projekte/Energielandingpage]]
- [[02 Projekte/Ausbilder-Landingpage]]
- [[02 Projekte/ipNginx]]
- [[02 Projekte/SchulOrdnungHems]]
- [[02 Projekte/SchutzkonzeptHems]]

## Struktur

- **00 Kontext/** — Profil, Schreibstil, Zielgruppe, Fokus
- **01 Inbox/** — Unverarbeitete Einträge
- **02 Projekte/** — Aktive Projekte mit Ziel und Enddatum
- **03 Bereiche/** — Laufende Verantwortungsbereiche
- **04 Ressourcen/** — Referenzwissen und Skills
- **05 Daily Notes/** — Tagesnotizen (YYYY-MM-DD.md)
- **06 Archiv/** — Abgeschlossenes

## Agenten & Skills

| Aufruf | Funktion |
|---|---|
| `/frontend-agent` | Vue 3 + TypeScript, SOLID |
| `/backend-agent` | Python + FastAPI, Clean Architecture |
| `/vue-solid` | Vue SOLID Architektur-Skill |
| `/python-solid` | Python SOLID Architektur-Skill |
| `/flutter-solid` | Flutter + Dart, Clean Architecture |
| `/tdd` | Test-Driven Development |
| `/ui-ux-pro-max` | UI/UX-Design für Web + Mobile |